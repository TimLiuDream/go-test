...
func main() {
	...
	executeIfNoErr(err, func() {
		fmt.Printf("The new index after seeking from currant with offset %d:%d\n", offset1, index1)
		n, err = reader1.Read(buf1)
	})
	executeIfNoErr(err, func() {
		fmt.Printf("Read(%d):%q\n", n, buf1[:n])
	...
	//示例5
	reader5a := strings.NewReader("MultiReader returns a Reader that's the logical concatenation of " +
