...
func main() {
	...
	genReader := func(reader io.Reader) singleHandler {
		return func() (data string, n int, err error) {
			...
			if !ok {
				err = errors.New("unsported reader")
				return
			}
			//读取数据
			mu.Lock()
